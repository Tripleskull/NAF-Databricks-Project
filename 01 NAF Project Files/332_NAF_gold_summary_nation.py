...
# MAGIC ),
# MAGIC
# MAGIC -- Opponent strength aggregated to nation level (valid GLO coaches only).
# MAGIC -- Reads from coach_performance_summary (331) + nation_coach_glo_metrics (330)
# MAGIC -- to filter by is_valid_glo without requiring opponent strength in 330.
# MAGIC opp_strength_by_nation AS (
# MAGIC   SELECT
# MAGIC     m.nation_id,
# MAGIC     AVG(cps.avg_opponent_glo_peak) AS avg_opponent_glo_peak
...
# MAGIC     AND cps.avg_opponent_glo_peak IS NOT NULL
# MAGIC   GROUP BY m.nation_id
# MAGIC )
# MAGIC
# MAGIC SELECT
# MAGIC   n.nation_id,
...
# MAGIC --                naf_catalog.gold_summary.nation_coach_glo_metrics (330)
# MAGIC -- NOTES        : Lives in 332 (not 331) because it depends on
# MAGIC --                nation_coach_glo_metrics (330) for opponent GLO peak.
